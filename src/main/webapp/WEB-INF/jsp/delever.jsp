...
<body>
	...
	<hr/>
	<div>
		<form name="orderform" action="/sizaikanri/delever/search" method="post">
			<s:select 
		...
		<table class="outputtable">
			...
			<s:iterator value="orderDeliverF.sizaiDtoList">
				<tr>
					...
					<s:if test="status == 4">
						<td><a href="http://localhost:8080/sizaikanri/delever/act?orderDeliverF.request_id=<s:property value="id"/>"><font>納品</font></a></td>
					</s:if>
					<s:if test="status != 4">
						<td><font color="red">納品済</font></td>
					</s:if>
					<s:if test="status == 5">
						<td><a target="under" href="http://localhost:8080/sizaikanri/delever/qr"><font>QRコード</font></a></td>
					</s:if>
					<s:if test="status != 5">
						<td><font></font></td>
					</s:if>
